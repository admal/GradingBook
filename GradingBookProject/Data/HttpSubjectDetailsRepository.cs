...
namespace GradingBookProject.Data
{
    ...
    /// </summary>
    class HttpSubjectDetailsRepository : HttpRepository<SubjectDetails, HttpSubjectDetailsRequestService>
    {
       // private HttpSubjectDetailsRequestService requestService = new HttpSubjectDetailsRequestService();
        ...
        /// <returns></returns>
        public new async Task EditOne(SubjectDetails grade)
        {
            if (await requestService.GetOne(grade.id) == null)
                throw new Exception("Such grade does not exist!");

            await requestService.UpdateOne(grade.id, new SubjectDetails
            {
                id = grade.id,
                sub_id = grade.sub_id,
